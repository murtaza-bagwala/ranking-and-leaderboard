...
class Tree {
  ...
  _insert(key, value, target) {
    ...
    if (key < target.key) {
      const newNode = this._insert(key, value, target.left);
      newNode.parent = target;
      target.left = newNode;
    } else {
      const newNode = this._insert(key, value, target.right);
      newNode.parent = target;
      target.right = newNode;
    }

    target._height = target.maxChildHeight() + 1;
    target._size = target.calculateSize();
